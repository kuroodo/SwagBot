	...
	public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
		...
		try {
			...
			if (message.startsWith("!join ") || ChatHelper.isMessageAnAudioQueue(message)) {
				...
				// Scans through the VoiceChannels in this Guild, looking for
				// one
			...
			// Start playing audio with our FilePlayer. If we haven't created
			// and
			...
			if (command.equals("!play") || ChatHelper.isMessageAnAudioQueue(message)) {
				...
				if (player == null || audioSourceType.equals("url") || audioSourceType.equals("file")
					...
					try {
						...
						// NOTE: You don't have to set the handler each time you
						...
						// (closeAudioConnection), however it would probably be
						// better
						// to pause the play back yourself before severing the
						// connection (If you are using a player class
						// you could just call the pause() method. Otherwise,
						// make
						// canProvide() return false).
						// Once again, you don't HAVE to pause before severing
						// an
						// audio connection,
						...
						// NOTE: "completely connected" is not just joining the
				...
				} else if (player.isStarted() && player.isStopped()) // If it
																		// did
																		// exist,
																		// has
																		// it
																		...
																		// before?
				{
					...
					return;
						...
						// Note:
						// if it was already playing, this will have no effect.
				{
					player.play();
					resetIdleCount();
				}
			...
			// You can't pause, stop or restart before a player has even been
			// created!
